...
{
    if (msg == channelTimer)   // keep track of channel state
    {
        delete msg;
        channelTimer = NULL;
    ...
    else if(msg->arrivedOn("phyInterface$i")) // a msg from outside world through
    {
        ...
        else  // Communication by other nodes. Causes channel availability adjustments.
        {
            ...
            switch(recMsg->getKind())
            {
                ...
                // add here
                case PUDATA:
                    ev<<"SS: PU is sending data. Keep Setting PU channel "<<recMsg->getProposedChannel()<<" as busy.\n";
                    channelState(recMsg->getProposedChannel(), 1);
                    delete recMsg;
                    break;
                case PUEND:
...
void drmSpecSensor::senseChannel(int type)   // NEED TO TAKE CARE OF WHEN TO CANCEL THESE TIMERS IN CASE THE NODE BECOMES RECEIVER.
{
    ...
    case SenseFreeCHANNEL:   // sense any free channel
        if(channelsArray[sensedChannel].getState() != 0)  // The currently sensed channels is either busy of occupied by PU
        {
            sensingIteration++;
            if (sensingIteration > 2*totalChannels)  // Convert the 3* into a parameter
            {
            ...
            else  // we can continue to look for next channel to sense
            {
                ...
                delete senseReply;
                ev<<"I am "<<myAddress<<"! Channel "<<sensedChannel+1<<" is busy. Sensing another; it is the "<<sensingIteration <<" iteration\n";
                sensedChannel++;
        ...
        {
            emit(sensingSignal, sensedChannel+1);
            ev<<"Channel "<<sensedChannel+1<<" is "<<channelsArray[sensedChannel].getState()<<" So it is free.\n"; currentDataChannel = sensedChannel+1;
    ...
    case SenseDataCHANNEL:   // sense the state of SU channel
        if(channelsArray[currentDataChannel-1].getState() == 0)  // data channel is still free.
        {
        ...
        break;
    case SenseProposedCHANNEL:
        if(channelsArray[proposedChannel-1].getState() == 0)
        {
            // Proposed channel is free
            currentDataChannel = proposedChannel;
            emit(sensingSignal, proposedChannel);
            ev<<"Proposed Channel "<<proposedChannel<<" is "<<channelsArray[proposedChannel-1].getState()<<". So it is free\n";
            senseReply->setChannelState(true);
        ...
        break;
    case SenseDrmCHANNEL:
        if(channelsArray[drmChannel-1].getState() == 0)            // DRM channel is free
        {
...
void drmSpecSensor::init()
{
    ...
    channelTimer = NULL;
    channelsArray = new chTimer[totalChannels];
